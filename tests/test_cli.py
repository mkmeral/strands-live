...
class TestCLI:
    ...
    @pytest.mark.asyncio
    async def test_main_success(self, mock_speech_agent_class):
        """Test the main function runs successfully."""
        # Mock the SpeechAgent class and its methods
        ...
        # Verify SpeechAgent was created with correct parameters
    ...
    @patch('src.cli.SpeechAgent')
    @pytest.mark.asyncio
    async def test_main_with_debug(self, mock_speech_agent_class):
        ...
        # Verify SpeechAgent was still created correctly
    ...
    @patch('src.cli.SpeechAgent')
    @pytest.mark.asyncio
    async def test_main_keyboard_interrupt(self, mock_speech_agent_class):
        """Test that main handles KeyboardInterrupt gracefully."""
        # Mock the SpeechAgent to raise KeyboardInterrupt
        mock_speech_agent = Mock()
        mock_speech_agent.initialize = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_main_general_exception(self, mock_print, mock_speech_agent_class):
        """Test that main handles general exceptions gracefully."""
        # Mock the SpeechAgent to raise a general exception
        mock_speech_agent = Mock()
    ...
    @pytest.mark.asyncio
    async def test_main_exception_with_debug(self, mock_traceback, mock_print, mock_speech_agent_class):
    ...
    def test_run_cli_without_debug(self, mock_asyncio_run, mock_parse_args):
        ...
        # Mock argument parsing to return no debug flag
        mock_args = Mock()
