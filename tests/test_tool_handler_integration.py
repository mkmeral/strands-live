...
class TestToolHandlerIntegration:
    ...
    async def test_speech_agent_tool_delegation(self):
        ...
        with patch('src.speech_agent.AudioStreamer'), \
            ...
            speech_agent.tool_handler.process_tool_use = AsyncMock(return_value={"result": "test"})
            
            # Should be able to call process_tool_use
            result = await speech_agent.process_tool_use("testTool", {"content": "test"})
            
            # Verify delegation occurred
            speech_agent.tool_handler.process_tool_use.assert_called_once_with("testTool", {"content": "test"})
    ...
    def test_new_features_dont_break_old_usage(self):
        ...
        schema = handler.get_tool_schema("getDateAndTimeTool")
    ...
    async def test_validation_does_not_break_valid_requests(self):
        ...
        for tool_name, content in valid_requests:
            ...
            assert "error" not in result or "Invalid tool request" not in result.get("error", "")
