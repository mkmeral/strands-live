...
class TestToolHandler:
    ...
    async def test_validate_tool_request_valid_date_time(self):
        ...
        is_valid = await self.tool_handler.validate_tool_request(
            "getDateAndTimeTool", {}
        )
    ...
    async def test_validate_tool_request_valid_track_order(self):
        ...
        tool_use_content = {"content": json.dumps({"orderId": "VALID123"})}
    ...
    async def test_validate_tool_request_invalid_track_order_no_order_id(self):
        ...
        tool_use_content = {"content": json.dumps({"otherField": "value"})}
    ...
    async def test_validate_tool_request_invalid_json(self):
        ...
        tool_use_content = {"content": "invalid json content"}
    ...
    async def test_validation_error_handling(self):
        ...
        tool_use_content = {"content": json.dumps({})}  # Missing orderId
    ...
    async def test_track_order_new_format(self):
        ...
        tool_use_content = {"orderId": "NEW123", "requestNotifications": True}
    ...
    async def test_track_order_format_compatibility(self):
        ...
        # New format
        new_format = {"orderId": order_id, "requestNotifications": False}
    ...
    async def test_validate_tool_request_new_format(self):
        ...
        # Valid new format
        valid_new_format = {"orderId": "VALID123", "requestNotifications": False}

        is_valid = await self.tool_handler.validate_tool_request(
            "trackOrderTool", valid_new_format
        )
        assert is_valid is True

        # Invalid new format - missing orderId
        invalid_new_format = {"requestNotifications": False}

        is_valid = await self.tool_handler.validate_tool_request(
            "trackOrderTool", invalid_new_format
        )
        assert is_valid is False
